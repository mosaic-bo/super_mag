...
class AdminProductController extends AdminBase {
		...
		if (isset($_POST['submit'])) {
			...
			if ($errors == false) {
				...
				if ($id) {
					...
					// Проверим, загружалось ли в форму изображение
			...
			// Перенаправляем пользвателя на страницу управления товарами
	...
	public function actionUpdate($id) {
		...
		// Получаем список категорий дл выпадающего списка
		$categoriesList = Category::getCategoriesListAdmin();
		...
		if (isset($_POST['submit'])) {
			...
			// Если форма отправлена
			// Получаем данные из формы редактирования. При необходимости можно редактивароть значения
			$options['name'] = $_POST['name'];
			$options['code'] = $_POST['code'];
			$options['price'] = $_POST['price'];
			$options['category_id'] = $_POST['category_id'];
