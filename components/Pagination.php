...
class Pagination {
	...
	/**
	 * Ключ для GET, в который придется номер страницы
	 */
	...
	public function get() {
		...
		# Получаем ограничения для списка
		$limits = $this->limits();
	...
	/**
	 * Для полученя, куда стартовать
	 * @return массив с началом и концом отсчета
	 */

	private function limits() {
		# Вычисляем ссылки слева (чтоба активная ссылка была посередине)
		...
		if ($start + $this->max <= $this->amount) {
			# Назначаем конец цикла вперед на $this->max страниц вперед или просто на минимум
